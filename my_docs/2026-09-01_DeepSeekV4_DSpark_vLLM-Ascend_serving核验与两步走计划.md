...
- **本文是上机计划，结论已被 [2026-09-03 的定位记录](2026-09-03_DeepSeekV4_DSpark_微调权重上机_定位记录与结论.md) 取代**：
  BF16 独立目录不可用、INT8 独立目录与 same-checkpoint 等效（55.8% vs 55.4%）、§3 覆盖方案下架、
  §5.5 的 (d)（router 漂移）与 §5.4 归因到的"小张量位移"都已被覆盖回官方值的实验单独排除。
  微调的真实代价是 **−11.3 点**（44.1% vs 55.4%），分布在全部大矩阵上，根因是短跑用了
  长跑的超参。两处冲突时以那份为准。
