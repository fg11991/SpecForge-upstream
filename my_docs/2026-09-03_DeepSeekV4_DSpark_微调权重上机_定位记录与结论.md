...
5. **量化导出器已落地并实测验收**：`scripts/export_dspark_ascend_w8a8.py`。官方权重经
   BF16 过一遍导出器再服务，拿到 **55.5%**——与基线 55.4%、与不过导出器的 55.8% 三者
   在 ±0.4 的噪声内。**导出器零损耗**，`bf16 → INT8` 这条往返也是无损的。
6. **微调的第一个公平成绩：44.1%**，比基线低 11.3 个点。链路上每一环都有对照，所以这
   11.3 个点没有别的去处——就是微调过的那 2343 个大矩阵。**微调确实伤了 drafter。**
7. **根因是短跑超参**：100 step × `accumulation_steps=48` = 4800 个 sequence-pass，
   数据 200 条 → 24 个 epoch，而学习率仍用着为 `max_steps=10000` 准备的 2e-4。
   见 §6。
...
| 13 | 导出器首次实跑（输入 `0901_export`） | 产出 W8A8 draft 目录 | 2378 → 7022 张量、6 分片、23.47 GB、`max_error_lsb = 0.5000019` | 0.5 LSB 是 round-to-nearest 的理论下界 |
| 14 | 服务 `0903_export`（混合体） | 微调大矩阵在 W8A8 下的成绩 | **44.1%** / 4.09 / 26.18 ms | 与基线差 11.3 点，但当时归因未定 |
| 15 | **identity 对照**：官方权重 → 导出器 → 服务 | 把"导出器损耗"和"微调损耗"分开 | **55.5%** / 4.89 / 22.48 ms | **导出器无损**，11.3 点全记在微调头上 |
...
| 官方权重 + 独立目录 + BF16 | **< 1%** | — | — | — |
| 官方权重 → BF16 → 导出器 → INT8 | **55.5%** | 4.89 | 90.5 / 78.5 / 66.4 / 54.7 / 43.6 / 32.6 / 22.4 | 22.48 ms |
| 微调大矩阵 + 官方 router/小张量 | **44.1%** | 4.09 | 85.2 / 69.2 / 53.2 / 40.6 / 29.6 / 19.2 / 11.6 | 26.18 ms |

第 1、2、4 行三者在 ±0.4 的噪声内：**serving 路径、独立目录、导出器都不吃分**。
第 3 行是同一批权重、同一个目录、同一条命令，只换成未量化 kernel。
第 5 行是唯一一个换了权重的，掉 11.3 点。
...
- 首次实跑（输入 `0901_export`，2378 个张量）：产出 7022 个张量、6 个分片、23.47 GB，
  `--verify` 报 `max_error_lsb = 0.5000019`。0.5 LSB 是 round-to-nearest 的**理论下界**
  （多出的 1.9e-6 是校验本身除以 scale 的 fp32 舍入），说明量化实现干净。
- 端到端：官方权重经 BF16 过一遍导出器再服务，**55.5%**，与基线 55.4%、与不过导出器的
  55.8% 三者同在 ±0.4 噪声内。**导出器零损耗，`bf16 → INT8` 往返无损。**

注意区分两个"1 LSB"：`--verify` 的 0.5 LSB 比的是「产物反量化 vs 输入的 BF16 权重」，
衡量导出器忠不忠实于输入；早先往返自检的 `max|dq| = 1` 比的是「我们的 INT8 码 vs 官方的
INT8 码」，两次独立量化各自在 0.5 LSB 内、码值就可能差 1。两者不矛盾。
...
服务 `0903_export`（官方 router + 官方 32 个小张量 + **微调过的 2343 个大矩阵**）：
**44.1% / 4.09 / TPOT 26.18 ms**，比基线低 11.3 个点，逐位曲线整体下移
（pos0 85.2% 对 89.7%）。

链路上每一环都有对照，所以这 11.3 个点没有别的去处：

| 环节 | 对照 | 结论 |
| --- | --- | --- |
| serving 路径 | 官方 same-checkpoint 55.4% | 基准 |
| 独立 draft 目录 | 官方 INT8 软链 55.8% | 不吃分 |
| 导出器 + bf16 往返 | 官方权重过导出器 55.5% | 不吃分 |
| **微调过的权重** | **44.1%** | **−11.3 点** |

**微调确实伤了 drafter。** 注意这个结论直到 identity 对照跑完才成立——在那之前，
44.1% 同样可以是导出器的损耗。

### 6.2 根因：为短跑用了长跑的超参

- `max_steps=100`、`accumulation_steps=48`、`batch_size=1`、EP=8/DP=1
  → 4800 个 sequence-pass；数据约 200 条 → **24 个 epoch**；
- `learning_rate: 2.0e-4` 是 recipe 给 `max_steps=10000` 全量训练准备的，
  warmup 4 步之后基本满速跑完 100 步，**Σlr ≈ 0.005~0.01**。

这与 §2.3 量到的完全吻合：每个张量的绝对位移都落在 0.004~0.011，与张量自身量级无关
（Adam 把梯度归一化，每步位移 ≈ lr）。对大矩阵是 1.2~2.6% 的相对扰动，对小量级参数
就是重写（`mtp.2.hc_head_fn` 29.4%）。24 个 epoch 的位移全用来记忆那 200 条了。

单独把 router 和小张量覆回官方值都救不回来（§2.2、§2.3），因为损伤是**分布在所有
大矩阵上的**，不集中在某一组参数。

### 6.3 建议的改法（按影响排序）

1. **学习率降 1~2 个数量级**（2e-5 甚至 5e-6）。判据是可测的：训练完跑
   `compare_draft_to_official.py --output`，`max_abs_diff` ≈ Σlr；要把大矩阵的相对
   扰动压到 0.2% 以内，位移得到 5e-4 量级。
2. **加数据**。200 条对一个 256 专家的 MoE drafter 太少——每个 expert 平均只看到
   6/256 的 token。要么加到几万条，要么就把这次定位成流程验证、不指望学到东西。
3. **`router_bias_update_rate` 设 0**（短期微调）。它按 optimizer step 计数，与 lr 和
   数据都无关，100 步就把 768 个 bias 推满了 ±0.1（§2.2）。它没造成这次的故障，
   但是个白送的扰动。
4. **冻结 mHC 或给它单独的小 lr**（`hc_attn/ffn/head` 的 fn/base/scale）。它们量级只有
   0.05~0.06，Adam 的等幅位移对它们伤害最大。

### 6.4 一条方法论

**训练侧的 acceptance 指标当时看着正常，服务端却掉了 11 个点。** 那个指标是
teacher-forced 的，对这次的损伤不敏感。调参循环里应该直接挂 200 条基准，不能只看
训练指标。
...
1. **按 §6.3 重训一版** → 导出 → 跑同一基准 → 与 55.4% 比。整条链路（训练 → 导出 →
   服务 → 评测）现在都通了且各环节都有对照，可以放心迭代。
2. 可选：把 35 个张量用两个 backup 还原成纯微调态再导一版，看 router 与小张量的漂移
   会不会在 44.1% 之上再叠几个点。对调参没有直接帮助，只是把账记全。
3. 若要给 vllm-ascend 报 BF16 那个 bug，按 §4 的最小复现准备材料。
...
3. 纯微调态（未还原那 35 个张量）的成绩没测过，44.1% 是混合体的数。
4. §6.3 的四条改法都还没有实验支撑——它们来自对位移量的测量，不是来自跑过的对照。
5. 容器里实际安装的 `vllm_ascend` 是否逐字等于 `dedbb34`。`DSpark draft model loaded: 77 params` 与静态推算逐一吻合，这是运行时的旁证，但不是全量比对。
